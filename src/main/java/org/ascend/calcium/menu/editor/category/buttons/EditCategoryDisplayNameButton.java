...
@AllArgsConstructor
public class EditCategoryDisplayNameButton extends Button {
    Category category;

    @Override
    public ItemStack getItemStack(Player player) {
        return ItemMaker.of(Material.BEACON)
                .setDisplayName("&6Edit Display Name")
                .setLore(
                        "&7This will allow you to change the",
                        "&7name of the category in the menu.",
                        "",
                        "&6- &7Current Display Name: &a" + category.getDisplayname(),
                        "",
                        "&6Left-Click &7to edit this categories display-name."
                )
                .build();
    }
    ...
    public static class EditNamePrompt extends StringPrompt {
        ...
        public String getPromptText(ConversationContext conversationContext) {
            return ChatColor.GOLD + "[Calcium] " + ChatColor.YELLOW + "Please type the new display name for the '" + ChatColor.WHITE + category.getName() + ChatColor.YELLOW + "' category. Type " + ChatColor.RED + "cancel" + ChatColor.YELLOW + " to cancel.";
        }
        ...
        public Prompt acceptInput(final ConversationContext conversationContext, final String input) {
            ...
            if (input.equalsIgnoreCase("cancel")) {
                ...
                conversationContext.getForWhom().sendRawMessage(CC.translate("&6[Calcium] &cYou have cancelled the process of editing the categories display name."));
                return EditNamePrompt.END_OF_CONVERSATION;
            }
            conversationContext.getForWhom().sendRawMessage(CC.translate("&6[Calcium] &aYou have successfully updated the display name of " + this.category.getName() + " &ato &6" + input + "&a."));
            category.setDisplayname(CC.translate(input));
            new EditCategoryMenu(category).openMenu(sender.getPlayer());
