...
@AllArgsConstructor
public class EditCategoryDefaultButton extends Button {
    Category category;

    @Override
    public ItemStack getItemStack(Player player) {
        return ItemMaker.of(getMaterial())
                .setDisplayName("&6Default Category")
                .setLore(
                        "&7This will allow make it so all",
                        "&7new tags appear in this category.",
                        "",
                        "&6- &7Current Status: &a" + (this.category.isGlobal() ? "&cFalse" : "&aTrue"),
                        "",
                        "&6Left-Click &7to toggle the default category."
                )
                .build();
    }
    ...
    public void onClick(Player player, int slot, ClickType clickType) {
        category.setDefault(!this.category.isDefault());
        Calcium.getInstance().getModuleManager().getCategory().saveCategory(category.getName());
        player.sendMessage(CC.translate("&6[Calcium] &aCategory Default status has been updated to &f" + (this.category.isGlobal() ? "&cFalse" : "&aTrue") + "&a."));
    }
